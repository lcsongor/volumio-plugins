...
IRControl.prototype.saveConfig = function (data) {
  ...
  self.commandRouter.pushToastMessage(
    'success',
    self.getI18nString('PLUGIN_CONFIGURATION'),
...
IRControl.prototype.setVolume = async function (newvolume) {
  ...
  if (self.desiredVolume < self.savedDesiredConfig.volume) {
    ...
    self.log(
      'Decreasing volume from ' +
        self.savedDesiredConfig.volume +
        ' to ' +
        self.desiredVolume +
        ' in ' +
        indexer +
...
IRControl.prototype.turnOffAmplifierWithDelay = async function () {
  ...
  if (!self.stopInProgress) {
    self.log(
      'Playback was stopped, amplifier will be turned off in ' +
        stopToTurnOffDelay +
        ' seconds'
    )
...
IRControl.prototype.statusChanged = function (state) {
  ...
  if (state.status == 'play') self.handleEvent(MUSIC_PLAY, state)
  else if (state.status == 'pause') self.handleEvent(MUSIC_PAUSE, state)
  else if (state.status == 'stop') self.handleEvent(MUSIC_STOP, state)
}
